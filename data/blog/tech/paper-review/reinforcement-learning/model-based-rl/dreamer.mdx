...
<img
  src="/static/images/paper-review/dreamer_latent_dynamics_model.png"
...
<img
  src="/static/images/paper-review/dreamer_behavior_in_imagination.png"
  className="mx-auto my-4 block w-[24em]"
/>
...
\text{Value Model:  } & v_\psi(s_\tau) \approx E_{q(\cdot \lvert s_\tau)} ( \sum_{\tau = t}^{t+H} \gamma^{\tau-t} r_\tau)
\end{align}
$$
...
$$
\begin{align}
V_R (s_\tau) &= E_{q_\theta, q_\phi} (\sum_{n=\tau}^{t+H} r_n)\\
V_N^k(s_\tau) &= E_{q_\theta, q_\phi} (\sum_{n=\tau}^{h-1} \gamma^{n-\tau} r_n + \gamma^{h-\tau} v_{\psi}(s_h) ) \quad h = \min (\tau+k, t+H)\\
V_\lambda(s_\tau) &= (1-\lambda) \sum_{n=\tau}^{h-1} \gamma^{n-\tau} V_N^n(s_\tau) + \lambda^{H-1}V_N^H(s_\tau)
\end{align}
$$
...
$$
\begin{align}
\text{Action Model Obj:  }& \max_\phi E_{q_\theta, q_\phi}(\sum_{\tau=t}^{t+H} V_{\lambda}(s_\tau) )\\
\text{Value Model Obj:  }& \min_\psi E_{q_\theta, q_\phi}(\sum_{\tau=t}^{t+H} {1 \over 2} \| v_\psi(s_\tau) - V_\lambda(s_\tau) \|^2  )
\end{align}
$$
...
<img
  src="/static/images/paper-review/dreamer_3_part_of_algorithm.png"
  className="mx-auto my-4 block w-[40em]"
/>
...
<img
  src="/static/images/paper-review/dreamer_algorithm.png"
  className="mx-auto my-4 block w-[40em]"
/>
