...
&\text{maximize} \ \eta \\
&\begin{align}
\text{where} \ &\eta(\pi) = E_{s_0, a_0 ...} [\sum_{t+0}^\infty \gamma^t r(s_t)] \\
...
$$
\begin{align}
E_{\tau \lvert \tilde \pi} [\sum_{t=0}^\infty \gamma^t A_\pi (s_t, a_t)]
&= E_{\tau \lvert \tilde \pi} [\sum_{t=0}^\infty \gamma^t Q_\pi(s_t, a_t) - V_\pi(s_t)] \\
&= E_{\tau \lvert \tilde \pi} [\sum_{t=0}^\infty \gamma^t (r(s_t) + \gamma V_\pi (s_{t+1}) - V_\pi (s_t))] \\
&= E_{\tau \lvert \tilde \pi} [-V_\pi(s_0) + \sum_{t=0}^\infty \gamma^t r(s_t)] \\
&= -E_{s_0}[V_\pi(s_0)] + E_{\tau \lvert \tilde \pi} [\sum_{t=0}^\infty \gamma^t r(s_t)] \\
&= - \eta(\pi) + \eta(\tilde \pi)
\end{align}
$$
...
$$
\eta(\tilde \pi) = \eta(\pi) + E_{s_0, a_0, ... \backsim \tilde \pi} [\sum_{t=0}^\infty \gamma^t A_\pi (s_t, a_t)]
$$

기존의 policy $$\pi$$에서 새로운 policy $$\tilde \pi$$로 업데이트되었다고 할 때 $$E_{s_0, a_0, ... \backsim \tilde \pi} [\sum_{t=0}^\infty \gamma^t A_\pi (s_t, a_t)]$$의 값이 음수면 업데이트의 결과 return이 줄어들고, 양수면 return이 늘어나는 것으로 예상할 수 있다.
...
$$
\begin{align}
    \eta(\tilde \pi) &= \eta(\pi) + \sum_{t=0}^\infty \sum_s P(s_t = s \lvert \tilde \pi) \sum_a \tilde \pi (a \lvert s) \gamma^t A_\pi (s,a) \\
    &= \eta(\pi) + \sum_s \sum_{t=0}^\infty \gamma^t P(s_t = s \lvert \tilde \pi) \sum_a \tilde \pi (a \lvert s) A_\pi (s,a) \\
    &= \eta(\pi) + \sum_s \rho_{\tilde \pi}(s) \sum_a \tilde \pi (a \lvert s) A_\pi (s,a)
\end{align}
$$

위의 식에서 모든 state $$s$$에 대해 $$\sum_a \tilde \pi (a \lvert s) A_\pi (s,a) \geqq 0$$ 이 성립하면 $$\eta$$가 증가하는 것을 보장할 수 있다. 이렇게 policy를 업데이트하는 대표적인 방법이 deterministic policy $$\tilde \pi(s) = \arg \max_a A_\pi (s,a)$$를 사용하는 policy iteration 이며, 이렇게 하면 optimal policy에 수렴할 수 있다.
...
$$
\begin{align}
&\eta(\tilde \pi) = \eta(\pi) + \sum_s \rho_{\tilde \pi}(s) \sum_a \tilde \pi (a \lvert s) A_\pi (s,a) \\
\rightarrow &L(\tilde \pi)= \eta(\pi) + \sum_s \rho_{\pi}(s) \sum_a \tilde \pi (a \lvert s) A_\pi (s,a)
\end{align}
$$
...
<img src="/static/images/paper-review/trpo_algorithm.png" className="mx-auto block w-[80%]" />
...
$$
\begin{align}
    &\text{maximize}_{\theta} \ \sum_s \rho_{\theta_{old}}(s) \sum_a \pi_{\theta}(a \lvert s) A_{\theta_{old}}(s,a)\\
    &\text{subject to} \ D_{\text{KL}}^{\rho_{\theta_{old}}} (\theta_\text{old}, \theta) \leqq \delta
\end{align}
$$
...
- Summation $$\sum_s \rho_{\theta_{old}}(s)[...]$$ $$\Rightarrow$$ Expecation $${1 \over 1 - \gamma} E_{s \backsim \rho_{\theta_{old}}}[...]$$
...
$$
\sum_a \pi_{\theta}(a \lvert s_n) A_{\theta_{old}}(s_n, a) = E_{a \backsim q}[{\pi_{\theta}[a \lvert s_n] \over q(a \lvert s_n)} A_{\theta_{old}}(s_n, a)]
$$
...
$$
E[f(x)] \approx {1 \over n} \sum_{i=1}^n f(x_i) \qquad \text{where, } x \backsim p, x_i \backsim p
$$
...
& \approx {1 \over n} \sum_{i=1}^n f(x_i) {p(x_i) \over q(x_i)} \qquad x_i \backsim q
\end{align}
$$
