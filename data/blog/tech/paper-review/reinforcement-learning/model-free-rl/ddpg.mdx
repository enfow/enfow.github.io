...
$$
\begin{align}
&\text{maximize}_{\theta} \ E_{s_t \backsim \rho^\beta, a_t \backsim \beta} [Q^\mu(s_t, a_t \lvert \theta^Q)] \\
&\text{subject to} \ a_t = \mu(s_t \lvert \theta^\mu)
\end{align}
$$
...
$$
\begin{align}
&\text{maximize}_{\theta^\mu} \ E_{s_t \backsim \rho^\beta} [Q^\mu(s_t, \mu(s_t \lvert \theta^\mu) \lvert \theta^Q)] \\
&\text{minimize}_{\theta^Q} \ E_{(s_t, a_t, r_t, s_{t+1}) \backsim \mathcal D} [(Q^\mu(s_t, a_t \lvert \theta^Q) - y_t)^2]
\end{align}
$$
...
<img src="/static/images/paper-review/ddpg_algorithm.png" className="mx-auto block w-full" />
