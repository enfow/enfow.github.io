...
date: '2021-10-17'
tags: ['Reinforcement-Learning', 'Parallel-RL']
draft: false
summary: ''
---
...
$$
\rho_{t : T - 1} = \frac{\Pi_{k=t}^{T-1} \pi(A_k \vert S_k) p(S_{k+1} \vert S_k, A_k)}{\Pi_{k=t}^{T-1} \mu(A_k \vert S_k) p(S_{k+1} \vert S_k, A_k)} = \Pi_{k=t}^{T-1} \frac{\pi (A_k \vert S_k)}{\mu (A_k \vert S_k)}
$$
...
$$
E_{a_t \sim \mu(\cdot \vert s_t)} [ \frac{\pi (a_t \vert s_t)}{\mu(a_t \vert s_t)} \nabla \log \pi(a_t \vert s_t) q_t \vert s_t]
$$
...
$$
\begin{align}
& c_i = \min (\bar c, \frac{\pi (a_i, \vert s_i)}{\mu (a_i, \vert s_i)})\\
& \rho_k = \min (\bar \rho, \frac{\pi (a_k, \vert s_k)}{\mu (a_k, \vert s_k)})
\end{align}
$$
...
$$
\pi_{\bar \rho} (a \vert s) =^{def} \frac{ \min (\bar \rho \mu (a \vert s), \pi(a \vert s))}{ \sum_{b \in A} \min (\bar \rho \mu (b \vert s), \pi(b \vert s))}
$$

만약 $$\bar \rho$$의 값이 무한이라면 $$\rho$$의 값은 항상 $$\frac{\pi (a, \vert s)}{\mu (a, \vert s)}$$ 가 된다. Importance Sampling Weight에 대한 Truncation이 전혀 되지 않는다는 것이고, 따라서 Value Function은 Target Policy $$\pi$$의 그것과 동일해진다. 반대로 $$\bar \rho$$의 값이 0에 가까워질수록 Behavior Policy $$\mu$$의 그것에 보다 가까워게 된다.
...
<img src="/static/images/paper-review/impala_vs_batch_a2c.png" className="mx-auto block w-[30em]" />
