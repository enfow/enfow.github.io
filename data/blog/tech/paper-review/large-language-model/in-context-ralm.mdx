...
논문을 읽으며 RALM과 RAG(Retrieval Augmented Generation)의 관계에 대해 궁금증이 생겨 검색을 해보았다. 매일 같이 새로운 용어들이 생겨나는 머신러닝 연구의 특성 상 용어 자체는 크게 중요하지 않다고 생각하지만, 정리된 글을 쓰기 위해 또는 연구 간의 비교를 위해서는 필요하다고 생각했다. 
