...
<img
  src="/static/images/paper-review/two_graph_is_same.png"
  className="mx-auto my-6 block w-[48em]"
/>
...
<img
  src="/static/images/paper-review/convolution_filter.png"
  className="mx-auto my-6 block w-[25em]"
/>
...
먼저 일반적으로 말하는 Laplacian Matrix $$L$$이란 다음과 같이 Degree Matrix에 Adjecency Matrux를 뺀 것이다. 참고로 Degree Matrix는 $$D_{ii} = \sum_j A_{ij}$$, 즉 Node $$i$$의 이웃 Node 갯수를 Diagonal Element로 갖는 Diagonal Matrix를 말한다.
...
$$
g_{\theta'}\star x \approx \sum_{k=0}^{K} \theta'_k T_k(\tilde \Lambda)x
$$
...
&= \theta'_0 x + \theta'_1(\frac{2}{\lambda_{\text{max}}} \Lambda - I_N)x \\
&= \theta'_0 x + \theta'_1(\frac{2}{2} \Lambda - I_N)x \qquad ... (\lambda_{\text{max}} = 2)\\
&= \theta'_0 x + \theta'_1((I_N - D^{-\frac{1}{2}}AD^{-\frac{1}{2}}) - I_N)x \\
&= \theta'_0 x + \theta'_1(D^{-\frac{1}{2}}AD^{-\frac{1}{2}})x \\
\end{align}
$$
...
$$
g_{\theta'}\star x \approx \theta (I_N + D^{-\frac{1}{2}}AD^{-\frac{1}{2}})x \qquad ... (\theta = \theta'_0 = -\theta'_1)
$$
...
<img
  src="/static/images/paper-review/graph_convolution_network_semi_superviced.png"
  className="mx-auto block w-[32em]"
/>
