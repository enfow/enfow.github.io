...
date: '2020-05-06'
tags: ['Anomaly Detection', 'Auto-Encoder']
draft: false
summary: ''
---
...
\begin{aligned}
L(\theta, \phi; x)
&= - D_{KL}(q_{\phi}(z \mid x) \| p_\theta(z \mid x)) + \log p_\theta(x) \\
&= - D_{KL}(q_\phi(z \mid x) \| p_\theta(z)) + \mathbb{E}_{z \sim q_\phi(z \mid x)}[\log p_\theta(x \mid z)]
\end{aligned}
$$
...
$$\text{recon probability(i)} = \frac{1}{L} \sum_{l=1}^L p_\theta (x^{(i)} \lvert \mu_{(\hat x)}^{(i,l)}, \sigma_{(\hat x)}^{(i,l)} ) $$
