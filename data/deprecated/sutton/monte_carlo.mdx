...
다양한 $$q(s,a)$$를 경헙하는 것과 관련해 Exploring Start와 Stochastic Policy 두 가지 방법이 있다고 했었다. **Monte Carlo ES**는 이 중  **Exploring Start**를 적용하여 Policy Iteration을 수행하는 방법이다.
...
<img
  src="/static/images/tech-blog/monte_carlo_es_algorithm.png"
  alt="monte_carlo_es_algorithm"
...
- On-Policy: Behavior Policy $$\mu(a|s) = \pi(a|s)$$
- Off-Policy: Behavior Policy $$\mu(a|s) \neq \pi(a|s)$$
...
<img
  src="/static/images/tech-blog/soft_policy_on_policy_algorithm.png"
  alt="soft_policy_on_policy_algorithm"
  className="mx-auto block w-3/4"
/>
...
$$
q_\pi(s, \pi'(s)) = \sum_a \pi'(a|s) q_\pi(s,a) = \frac{\epsilon}{|A(s)|} \sum_a q_\pi(s, a) + (1 - \epsilon) \max_a q_\pi(s,a) \geq v_\pi(s)
$$
...
$$
\rho^T_t = \frac{\prod_{k=t}^{T-1} \pi(A_k|S_k) p(S_{k+1}|S_k, A_k)}{\prod_{k=t}^{T-1} \mu(A_k|S_k) p(S_{k+1}|S_k, A_k)} = \frac{\prod_{k=t}^{T-1} \pi(A_k|S_k)}{\prod_{k=t}^{T-1} \mu(A_k|S_k)}
$$
...
$$
V(s) = \frac{\sum_{t \in \tau(s)} \rho_t^{T(t)} G_t}{|\tau(s)|}
$$

Importance Sampling Ratio에 가중치를 적용할 수도 있는데, 이를 **Weighted Importance Sampling**이라고 한다.

$$
V(s) = \frac{\sum_{t \in \tau(s)} \rho_t^{T(t)} G_t}{\sum_{t \in \tau(s)} \rho_t^{T(t)}}
$$
...
$$
V_{n+1} = V_n + \frac{W_n}{C_n} [G_n - V_n] \text{ where } C_{n+1} = C_n + W_{n+1}, C_0 = 0, n \geq 1
$$

$$C_n$$이 누적 $$W_k$$라는 점을 생각하며 위 식을 다음과 같이 변경하면 보다 직관적으로 이해할 수 있다.

$$
V_{n+1} = V_n + \frac{W_n}{C_n} [G_n - V_n] = V_n - \frac{W_n}{C_n} V_n + \frac{W_n}{C_n} G_n
$$

<img
  src="/static/images/tech-blog/incremental_monte_carlo_off_policy_algorithm.png"
  alt="incremental_monte_carlo_off_policy_algorithm"
  className="mx-auto block w-3/4"
/>
